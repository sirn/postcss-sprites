...
test("should update CSS declarations", async t => {
    ...
    [opts, images, spritesheets] = await mapSpritesheetProps(
        t.context.opts,
        images,
        spritesheets,
    );
    [root, opts, images, spritesheets] = await updateReferences(
        root,
        t.context.opts,
        images,
        spritesheets,
...
test("should use function provided by onUpdateRule hook", async t => {
    ...
    const expected = await readFileAsync(
        "./expectations/basic-on-update-rule-hook/style.css",
        "utf8",
    );
    const ast = postcss.parse(input, { from: "./fixtures/basic/style.css" });
    let images, spritesheets, opts, root;
