...
test("should save spritesheets", async t => {
    ...
    const ast = postcss.parse(cssContents, {
        from: "./fixtures/basic/style.css",
    });
    ...
    [opts, images, spritesheets] = await saveSpritesheets(
        t.context.opts,
        images,
        spritesheets,
    );
...
test("should save SVG spritesheets", async t => {
    ...
    const ast = postcss.parse(cssContents, {
        from: "./fixtures/svg-basic/style.css",
    });
    let images, spritesheets, opts;
...
test("should save spritesheets by groups", async t => {
    ...
    const ast = postcss.parse(cssContents, {
        from: "./fixtures/retina/style.css",
    });
    let images, spritesheets, opts;
        ...
        return new Promise(resolve =>
            setTimeout(
                () =>
                    resolve(
                        Promise.resolve(
                            path.join(pluginOpts.spritePath, "custom-name.png"),
                        ),
                    ),
                0,
            ),
